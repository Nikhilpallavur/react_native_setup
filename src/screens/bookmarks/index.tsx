...
interface BookmarkPageProps {
  navigation: NavigationProp<RootStackParamList, 'bookmark'>;
}

const BookmarkScreen = ({navigation}: BookmarkPageProps) => {
  ...
  const renderItem = ({item}: {item: ArticleType}) => {
    return (
      <PostCard
        ...
        userBookmarked={item?.userLiked}
        userLiked={item?.userBookmarked}
        onLikeToggle={() => {
  ...
  return (
    <AppMainLayout title="Bookmarks" showBackButton>
      <FlatList
        style={{flex: 1}}
        data={articles}
